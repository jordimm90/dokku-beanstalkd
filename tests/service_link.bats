...
@test "($PLUGIN_COMMAND_PREFIX:link) error when the service is already linked to app" {
  ...
  echo "output: $output"
  echo "status: $status"
  assert_contains "${lines[*]}" "Already linked as BEANSTALKD_URL"
  assert_failure

  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) exports BEANSTALKD_URL to app" {
  run dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  echo "output: $output"
  echo "status: $status"
  url=$(dokku config:get my-app BEANSTALKD_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
  assert_contains "$url" "beanstalkd://beanstalkd:$password@dokku-beanstalkd-ls:5432/ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when BEANSTALKD_URL already in use" {
  dokku config:set my-app BEANSTALKD_URL=beanstalkd://user:pass@host:5432/db
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_BEANSTALKD_AQUA_URL"
  assert_success

  dokku "$PLUGIN_COMMAND_PREFIX:link" ms my-app
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_BEANSTALKD_BLACK_URL"
  assert_success
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.beanstalkd.ls:dokku-beanstalkd-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps BEANSTALKD_DATABASE_SCHEME variable" {
  dokku config:set my-app BEANSTALKD_DATABASE_SCHEME=beanstalkd2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app BEANSTALKD_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
  assert_contains "$url" "beanstalkd2://beanstalkd:$password@dokku-beanstalkd-ls:5432/ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
